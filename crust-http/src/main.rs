...
async fn main() {
    ...
    let state = crdt_requests::AppState::new();
    let api = Router::new()
        .route(
            "/crust/:type/operation/:operation",
            get(crdt_requests::get_operation),
        )
        .route("/crust/:type/state/:state", get(crdt_requests::get_state))
        .route("/crust/:type/delta/:delta", get(crdt_requests::get_delta));
